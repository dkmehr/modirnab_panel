      ...
      <div className="printArea fishPrintArea">
        <div className="userInfo hesabSection">
          ...
          <div className="hesabfaSection hesabBorder">
            <small>
              {" "}
              تاریخ: {new Date(orderInfo.orderDate).toLocaleDateString("fa")}
            </small>
            <small>
              ساعت: {new Date(orderInfo.orderDate).toLocaleTimeString("fa")}
            </small>
          </div>
        </div>
          ...
          <tbody>
            <tr>
              ...
              <td colSpan={3} className="hesabfaItem">
                <span>نام: </span>
                <strong> {userInfo && userInfo.cName}</strong>
                {/*+" ("+userInfo.Code+")"*/}
                <br />
          ...
          <tbody>
            <tr>
              ...
              <td className="priceCell">
                {normalPriceCount(orderData.cartPrice)}
              </td>
            </tr>
            <tr>
              <td>مالیات </td>
              <td className="priceCell">
                {normalPriceCount(orderData.cartPrice, TAX)}
              </td>
            </tr>
            <tr>
              <td>قابل پرداخت </td>
              <td className="priceCell">
                <b>{normalPriceCount(orderData.cartPrice, 1 + TAX)}</b>
              </td>
            </tr>
          </tbody>
        </table>
